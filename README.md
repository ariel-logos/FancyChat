...
Both pull from `addons/fancychat/notifications/*.wav`. Drop your own `.wav` files in there to add them to the dropdowns; files named `notification_<n>.wav` are picked up automatically. Each base file can also have a `..._<n>B.wav` boosted variant for the Volume Boost option.
